...
import { updateUserProfile, createPet, getUserProfile, getPet } from '@/lib/db';
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(async (user) => {
      ...
      setUid(user.uid);
      // A user who already finished onboarding must not re-run it — handleFinish
      // calls createPet() at level 1 and would wipe their progress. Send them home.
      try {
        const profile = await getUserProfile(user.uid);
        if (profile?.onboardingComplete) router.replace('/home');
      } catch {
        // If the profile read fails, let onboarding proceed rather than trap the user.
      }
    });
    return unsub;
  }, [router]);
  ...
  const handleFinish = async () => {
    ...
    setSaveError('');
    try {
      // Fail closed against overwriting an existing pet: if this account already
      // has one (e.g. it reached this screen via a stale/refreshed session), go
      // home instead of resetting progress to level 1. A read failure throws into
      // the catch below, so we never blindly overwrite.
      const existingPet = await getPet(uid);
      if (existingPet) {
        router.replace('/home');
        return;
      }
      // Create the pet first, then flip onboardingComplete — so a failure never
      // leaves a "complete" profile with no pet (which would break /home).
      await createPet(uid, {
        name: petName.trim(),
        color: selectedColor,
        level: 1,
      ...
      setSaveError("We couldn't save your capybara. Please try again.");
    } finally {
      setSaving(false);
    }
  };
    ...
    if (step === 7 + mobileOffset) {
      return (
        ...
        >
          ...
          {saveError && (
            <p className="text-failure text-xs mt-3" style={{ fontFamily: 'var(--font-body)' }}>
              {saveError}
            </p>
          )}
        </motion.div>
