...
function Processing({ text, sub }: { text: string; sub: string }) {
  return (
    <div role="status" aria-live="polite" className="mt-4 flex items-center gap-3 rounded-xl bg-coral/5 px-4 py-3">
      <span className="flex gap-1" aria-hidden="true">
        {[0, 1, 2].map((i) => (
          <span key={i} className="w-1.5 h-1.5 rounded-full bg-coral animate-pulse" style={{ animationDelay: `${i * 0.2}s` }} />
        ))}
      </span>
      <div>
        <p className="text-xs font-semibold text-near-black/80" style={BODY}>{text}</p>
        <p className="text-[11px] text-near-black/45" style={BODY}>{sub}</p>
      </div>
    </div>
  );
}
...
export default function StartPage() {
  ...
  /** Which textarea to focus once the next phase view has actually mounted. */
  const focusTarget = useRef<'work' | 'composer' | null>(null);
  ...
  const review = async () => {
    ...
    try {
      ...
      // Let the observation and the quote land first; the celebration follows.
      if (commit.awarded > 0) {
  ...
  // ── Composer (shared by conversation + proposal) ───────────────
  const composer = (
    <div
      {...contextZone.getRootProps()}
      className={`glass-strong rounded-2xl p-3 transition-shadow focus-within:ring-2 focus-within:ring-coral/40 ${contextZone.isDragActive ? 'ring-2 ring-coral/40' : ''}`}
    ...
    >
      ...
      <textarea
        ...
        className="w-full bg-transparent px-2 py-2 text-sm text-near-black placeholder-near-black/30 resize-none outline-none"
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        <button
          ...
          disabled={!canSend}
          aria-label="Send to Capy"
          className={`btn-hover flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
            canSend ? 'bg-coral text-white' : 'bg-near-black/10 text-near-black/30 cursor-not-allowed'
          }`}
        >
          <ArrowUp size={18} />
        </button>
  ...
  return (
    <MotionConfig reducedMotion={reduced ? 'always' : 'user'}>
      <div className="gradient-start min-h-screen">
          ...
          <div className="grid lg:grid-cols-[minmax(240px,0.8fr)_minmax(0,1.3fr)] gap-5 lg:gap-8 items-start">
              ...
              <div className="glass-strong rounded-2xl p-5 lg:p-6" style={{ boxShadow: 'var(--shadow-md)' }}>
                <div className="flex lg:flex-col items-center gap-4 lg:gap-3">
                  <motion.div
                    ...
                    animate={busy && !reduced ? { y: [0, -5, 0], scale: [1, 1.02, 1] } : { y: 0, scale: 1 }}
                    transition={busy ? { duration: 3.2, repeat: Infinity, ease: 'easeInOut' } : { duration: 0.4 }}
                  >
                    <CapyPet
            ...
            >
              ...
              <AnimatePresence mode="wait">
                {/* ── Welcome ── */}
                {!started && (
                  <motion.div key="welcome" initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.45 }}>
                ...
                {started && (phase === 'conversation' || phase === 'proposal') && (
                  <motion.div key="conversation" initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.35 }}>
                    {phase === 'proposal' && start.messages.length > 1 && (
                      <details className="mb-4 text-xs text-near-black/55" style={BODY}>
                        ...
                        <div className="mt-2 space-y-2 pl-1">
                          {start.messages.slice(0, -1).map((m, i) => (
                    ...
                    <section className={`space-y-4 ${phase === 'conversation' ? 'max-h-[46vh] overflow-y-auto pr-1' : ''}`} aria-label="Conversation">
                      {(phase === 'proposal' ? start.messages.slice(-1) : start.messages).map((m, i) => (
                        <motion.div
                        ...
                        >
                          ...
                          <p className={`${m.role === 'user' ? 'text-sm text-near-black/85' : 'text-sm sm:text-base text-near-black/85'} whitespace-pre-wrap break-words mt-1 leading-relaxed`} style={BODY}>
                            {m.text}
                          </p>
                    ...
                    {phase === 'proposal' && start.proposal && (
                      <motion.article
                        ...
                        className="glass-strong rounded-2xl p-5 sm:p-6 mt-5 border border-coral/20"
                      ...
                      >
                        ...
                        <p className="text-sm sm:text-base text-near-black/80 leading-relaxed" style={BODY}>{start.proposal.action}</p>
                        <dl className="mt-4 grid grid-cols-[88px_1fr] gap-x-3 gap-y-2 text-xs" style={BODY}>
                        ...
                        <div className="mt-4 pt-4 border-t border-coral/10 flex items-start gap-2 text-xs text-near-black/65" style={BODY}>
                ...
                {/* ── Working ── */}
                {phase === 'working' && agreement && (
                  <motion.div key="working" initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }}>
                ...
                {/* ── Result ── */}
                {phase === 'result' && start.review && (
                  <motion.div key="result" initial={{ y: 14, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.4 }}>
                    <p className="text-[10px] uppercase tracking-widest text-near-black/40 font-semibold mb-2" style={BODY}>Our check-in</p>
                    <h1 className="text-3xl sm:text-4xl text-near-black leading-tight mb-3">
                    ...
                    <p className="text-sm sm:text-base text-near-black/75 leading-relaxed mb-4" style={BODY}>{start.review.observation}</p>

                    {start.review.quote ? (
                      <blockquote className="rounded-r-2xl border-l-[3px] border-amber bg-amber/10 px-5 py-4 mb-4">
                    ...
                    <div className={`rounded-xl px-4 py-3 flex items-start gap-3 mb-4 ${start.review.status === 'supported' ? 'bg-success/10' : 'bg-near-black/5'}`}>
                      {start.review.status === 'supported' ? (
                    ...
                    <p className="text-sm text-near-black/80 leading-relaxed mb-5" style={BODY}>{start.review.next}</p>
                    ...
                    <details className="mt-6 text-xs text-near-black/55" style={BODY}>
                ...
                {busy && busy !== 'save' && (
                  <motion.div key="busy" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <Processing
                      ...
                      sub={busy === 'review' ? 'Checking it against what we agreed.' : 'Using what you’ve shared with me.'}
                    />
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              {error && (
                <div role="alert" className="mt-4 flex items-start gap-3 rounded-xl border border-failure/30 bg-failure/5 px-4 py-3 text-xs text-failure leading-relaxed" style={BODY}>
                  <span className="flex-1">{error}</span>
                  <button type="button" onClick={() => setError('')} aria-label="Dismiss" className="flex-shrink-0 mt-0.5 hover:opacity-70">
                    <X size={14} />
                  </button>
                </div>
              )}
              ...
              {lastCapy && (
                <span className="sr-only" role="status" aria-live="polite">{lastCapy.text}</span>
              )}
            </motion.section>
          </div>
        </div>
